package com.example.AppointmentCalendar.View.CustomView;
...
import com.example.AppointmentCalendar.View.Fragment.FirstFragment;
...
public class CustomCalenderView extends LinearLayout {
    ...
    private HashMap<LocalDate, ArrayList<Bookings>> eventusers = new HashMap<>();
    ...
    public void init(HashMap<LocalDate, String[]> eventhashmap, LocalDate mindate, LocalDate maxdate, ArrayList<Booking> _appointmentbooking) {
            ...
            for (String s : localDateStringEntry.getValue()) {
                ...
                if (type == 2 && eventModelslist.get(eventModelslist.size() - 1).getType() == 0 && eventModelslist.get(eventModelslist.size() - 1).getLocalDate().equals(localDateStringEntry.getKey())) {
                    ...
                    if (type == 0 && eventModelslist.size() > 0 && eventModelslist.get(eventModelslist.size() - 1).getType() == 0 && !eventModelslist.get(eventModelslist.size() - 1).getLocalDate().equals(localDateStringEntry.getKey())) {
                        ...
                        eventModelslist.add(new EventModel("dup", localDateStringEntry.getKey(), 100, 0));
                        i++;
                    } else if ((type == 3) && eventModelslist.size() > 0 && eventModelslist.get(eventModelslist.size() - 1).getType() == 0) {
                        eventModelslist.add(new EventModel("dup", eventModelslist.get(eventModelslist.size() - 1).getLocalDate(), 100, 0));
                        i++;
                    } else if ((type == 1) && eventModelslist.size() > 0 && eventModelslist.get(eventModelslist.size() - 1).getType() == 0) {
                        eventModelslist.add(new EventModel("dup", eventModelslist.get(eventModelslist.size() - 1).getLocalDate(), 200, 0));
                        i++;
                    } else if (type == 0 && eventModelslist.size() > 0 && (eventModelslist.get(eventModelslist.size() - 1).getType() == 1)) {
                        eventModelslist.add(new EventModel("dup", localDateStringEntry.getKey(), 200, 0));
                        i++;
                    ...
                    if(MainActivity.localDateHashMap.containsKey(localDateStringEntry.getKey())){
                        for(int customer=0;customer<MainActivity.localDateHashMap.get(localDateStringEntry.getKey()).size();customer++){
                            if(MainActivity.localDateHashMap.get(localDateStringEntry.getKey()).get(customer).getCustomer().customerName.equalsIgnoreCase(s)){
                                eventModelslist.add(new EventModel(s, localDateStringEntry.getKey(), type,MainActivity.localDateHashMap.get(localDateStringEntry.getKey()).get(customer).getBookingStartTime()));
                                indextrack.put(localDateStringEntry.getKey(), i);
                            }
                        }
                    }else{
                        eventModelslist.add(new EventModel(s, localDateStringEntry.getKey(), type,Long.parseLong("1579084476000")));
                        indextrack.put(localDateStringEntry.getKey(), i);
                    }
